...
var core = __webpack_require__(710);

// EXTERNAL MODULE: ./node_modules/@actions/http-client/index.js
var http_client = __webpack_require__(273);

// EXTERNAL MODULE: ./node_modules/@actions/http-client/auth.js
var auth = __webpack_require__(849);

// CONCATENATED MODULE: ./src/runtimeHttpClient.js




class runtimeHttpClient_RuntimeHttpClient {
    /**
    ...
    constructor() {
        ...
        this.client = new http_client.HttpClient('action/artifact', [
            new auth.BearerCredentialHandler(process.env['ACTIONS_RUNTIME_TOKEN'])
        ]);
    ...
    async deleteArtifact(url) {
        try {
            ...
            if (this.isStatusCodeSuccess(response.message.statusCode)) {
                ...
                Object(core.info)(`Failed to delete artifact; status code ${response.message.statusCode}`);
            }
        } catch (e) {
            Object(core.debug)(e);
        }
    ...
    async listArtifacts() {
        try {
            ...
            if (this.isStatusCodeSuccess(response.message.statusCode)) {
                ...
                Object(core.info)(`Failed to list artifacts; status code ${response.message.statusCode}`);
            }
        } catch (e) {
            Object(core.debug)(e)
        }
 ...
 */
function getNames() {
    return Object(core.getInput)('name')
        .split(/\r?\n/)
        .filter(name => name)
        .map(name => name.trim());
}
 ...
 */
function fail(msg) {
    const failOnError = Object(core.getInput)('failOnError');

    if (failOnError.toLowerCase() === 'true' || failOnError === '1') {
        Object(core.setFailed)(msg);
    } else {
        Object(core.error)(msg);
    }
}
 ...
 */
async function run() {
    const client = new runtimeHttpClient_RuntimeHttpClient(process.env);
    ...
    for (const name of getNames()) {
        ...
        if (artifact) {
            ...
            if (del.success) {
                Object(core.info)(`Successfully deleted artifact "${name}".`);
            } else {
                Object(core.error)(`Failed to delete artifact "${name}"; debug logs may be available.`);
                success = false;
            }
        } else {
